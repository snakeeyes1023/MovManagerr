...
namespace MovManagerr.Core.Helpers.Parsers.Langs
{
    public static class LanguageParser
    {
        ...
        public static IEnumerable<string> ParseLanguageTags(string fileName)
        {
            try
            {
                ...
                var languageTags = match.Groups["tags"].Captures.Cast<Capture>()
                    .Where(tag => !(tag.Value == string.Empty))
                    .Select(tag => tag.Value.ToLower());
                return languageTags;
            }
            catch (Exception ex)
            {
                SimpleLogger.Debug(ex, "Failed parsing language tags from subtitle file: {0}", fileName);
            }
        ...
        public static Language ParseSubtitleLanguage(string fileName)
        {
            try
            {
                SimpleLogger.Debug("Parsing language from subtitle file: {0}", fileName);
                ...
                SimpleLogger.Debug("Unable to parse langauge from subtitle file: {0}", fileName);
            }
            catch (Exception)
            {
                SimpleLogger.Debug("Failed parsing langauge from subtitle file: {0}", fileName);
            }
