...
using MovManagerr.Core.Helpers.Parsers.Generals;
using MovManagerr.Core.Helpers.Parsers.Helpers;
using MovManagerr.Core.Helpers.Parsers.Langs;
using MovManagerr.Core.Helpers.Parsers.Quality;
...
namespace MovManagerr.Core.Helpers.Parsers
{
    public static class Parser
    {
        ...
        public static ParsedMovieInfo ParseMoviePath(string path)
        {
            ...
            if (result == null)
            {
                SimpleLogger.Debug("Attempting to parse movie info using directory and file names. {0}", fileInfo.Directory.Name);
                result = ParseMovieTitle(fileInfo.Directory.Name + " " + fileInfo.Name);
            }

            if (result == null)
            {
                SimpleLogger.Debug("Attempting to parse movie info using directory name. {0}", fileInfo.Directory.Name);
        ...
        public static ParsedMovieInfo ParseMovieTitle(string title, bool isDir = false)
        {
            ...
            try
            {
                ...
                SimpleLogger.Debug("Parsing string '{0}'", title);
                ...
                if (ReversedTitleRegex.IsMatch(title))
                {
                    ...
                    SimpleLogger.Debug("Reversed name detected. Converted to '{0}'", title);
                }
                ...
                foreach (var replace in PreSubstitutionRegex)
                {
                    ...
                    {
                        SimpleLogger.Trace($"Replace regex: {replace}");
                        SimpleLogger.Debug("Substituted with " + releaseTitle);
                    }
                }
                ...
                {
                    if (QualityParser.ParseQualityName(m.Value).Quality != Quality.Quality.Unknown)
                    {
                        return string.Empty;
                    }
                    ...
                    if (match.Count != 0)
                    {
                        ...
                        try
                        {
                            ...
                            if (result != null)
                            {
                                ...
                                SimpleLogger.Debug("Release Group parsed: {0}", result.ReleaseGroup);
                                ...
                                SimpleLogger.Debug("Languages parsed: {0}", string.Join(", ", result.Languages));

                                result.Quality = QualityParser.ParseQuality(title);
                                SimpleLogger.Debug("Quality parsed: {0}", result.Quality.ToString());

                                if (result.Edition.IsNullOrWhiteSpace())
                                {
                                    result.Edition = ParseEdition(simpleReleaseTitle);
                                    SimpleLogger.Debug("Edition parsed: {0}", result.Edition);
                                }

                                result.ReleaseHash = GetReleaseHash(match);
                                if (!result.ReleaseHash.IsNullOrWhiteSpace())
                                {
                                    SimpleLogger.Debug("Release Hash parsed: {0}", result.ReleaseHash);
                                }
                        ...
                        {
                            SimpleLogger.Debug(ex, ex.Message, string.Empty);
                            break;
                        }
                    }
                }
            ...
            catch (Exception e)
            {
                ...
                {
                    SimpleLogger.Error(e, "An error has occurred while trying to parse {0}", title);
                }
            }

            SimpleLogger.Debug("Unable to parse {0}", title);
            return null;
        }
        ...
        private static ParsedMovieInfo ParseMovieMatchCollection(MatchCollection matchCollection)
        {
            ...
            SimpleLogger.Debug("Movie Parsed. {0}", result.ToString());
        ...
        private static bool ValidateBeforeParsing(string title)
        {
            ...
            {
                SimpleLogger.Debug("Rejected Hashed Release Title: " + title);
                return false;
            }
