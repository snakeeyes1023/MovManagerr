...
namespace MovManagerr.Core.Helpers.Parsers.Quality
{
    public class QualityParser
    {
        ...
        {
            SimpleLogger.Debug("Trying to parse quality for {0}", name);
            ...
            // Based on extension
            if (result.Quality == Quality.Unknown && !ContainsInvalidPathChars(name))
            {
                try
                {
        ...
        public static bool ContainsInvalidPathChars(string text)
        {
        ...
        public static QualityModel ParseQualityName(string name)
        {
            ...
            if (resolution != Resolution.Unknown)
            {
                ...
                else
                {
                    ...
                    {
                        //SimpleLogger.Debug(ex, "Unable to parse quality from extension");
                    }
                }
